...
### Immediate next = REBUILD THE FRONTEND EXPERIENCE (within the theme)

> **UPDATE 2026-06-02 (supersedes the original frontend plan).** The frontend was clean-rebuilt AND elevated to ~9/10 via a multi-agent senior audit (Next 16 / React 19 / Tailwind v4, branch `feat/frontend-rebuild`, commits `f1823a9` → `c8dc9ee` → `d8e703a` → `f41c593` → `5d50d91`, NOT pushed; tsc + eslint + build green; runs on a built-in mock). The founder reviewed it (and 4 theme alternatives) and decided to **KEEP the current Customs-Ledger theme and rebuild the EXPERIENCE**: a first-run preview/intro that demonstrates the workflow, a simpler-but-complete hero, and a signature classify→loading→result, all research-backed ("signs"). **Authoritative brief: `frontend/NEXT-UX-DIRECTION.md`** (plus `frontend/HANDOFF.md`; audit record in `design-mocks/`). Backend Step 3 (paid validation) + build-zero DB provisioning still wait for a user-enabled capped Tier-1 key.
...
1. **Frontend EXPERIENCE rebuild (current focus):** the frontend is BUILT + elevated (~9/10) on `feat/frontend-rebuild` (NOT pushed). NEXT = make the experience premium WITHIN the existing Customs-Ledger theme — a first-run preview/intro that demonstrates the workflow, a simpler-but-complete hero, and a signature classify→loading→result ("a beautiful thing they've never seen"; result "worth the wait"), all research-backed ("signs"). **Authoritative brief: `frontend/NEXT-UX-DIRECTION.md`.** The 12 locked decisions are largely realized; the honesty constraints carry forward. (The original clean-rebuild + hero sign-off sequence is done.)
