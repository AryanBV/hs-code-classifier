...
- **Step 1 LATENCY FIX — DONE (commit `0ae224a`).** Adaptive repair loop = cap repairs at 2 + code-only no-progress bail. r19→r21 three-sided gate (master 343 gold denom, `--simulate-answers`): **p95 62.5s → 38.8s (-38%)**, median 30.4s → 26.8s; OUTRIGHT 8-digit 76.5% → 76.4% (McNemar p=1.0000, identical); confident-wrong 65 → 71 (+6, run-to-run noise, user-accepted); ≥2-repair cases 161→6, bailed 0→164. Env knobs: `REPAIR_MAX_ITERATIONS` (2), `REPAIR_NOPROGRESS_BAIL` (on), `REPAIR_BAIL_ON_SIGNATURE` (off/opt-in), `REPAIR_BAIL_MIN_ITERATION` (0). r20 (sig+code bail) and r22 (cap=3) tested+REJECTED (r20 broke still-converging DB064/070/080; r22 over-repairs, McNemar p=0.049).
- **Remaining ship arc / next: stream → cutover → frontend rebuild (plan-with-user) → publish.** (2) stream HTTP response (`backend/src/api/classify.ts` + frontend); (3) staged cutover (`USE_V2_CLASSIFIER` ON); (4) **frontend rebuild — MUST be planned with user first** (`frontend/src/lib/hooks/use-wizard.ts`: read `alternatives`, handle `responseType:refused`, multi-turn `/answer` with `{questionId, answerId}`); (5) publish + trade intelligence. Deferred quality lever: calibrated escalation to route low-confidence best-effort emits to ASK (would recover the confident-wrong delta).

### Key commits this session (`feat/phase-4-pipeline-build`)
- gold-freeze R4 `6d9afd9`; MV-03 source_ref grammar `881ef2e`; bad-gold R5 `168ac64`; calibrated-classify lever (env-off) `8a189ef`; L2 `direct_leaf_lookup` recall `d4cfb44`; v2 API adapter + flag `cd7a5a3`; residual-leaf-floor `7629a2b`; top-3 instrumentation `ec34ee6`; adaptive repair-loop latency fix (cap=2 + code-only bail) `0ae224a`.
...
1. **DONE — Latency fix (ship Step 1, commit `0ae224a`):** adaptive repair loop = cap repairs at 2 + code-only no-progress bail. r19→r21 three-sided gate (master 343 gold denom, `--simulate-answers`): **p95 62.5s → 38.8s (-38%)**, median 30.4s → 26.8s; OUTRIGHT 8-digit 76.5% → 76.4% (McNemar p=1.0000, statistically identical); confident-wrong 65 → 71 (+6, within run-to-run noise, user-accepted); ≥2-repair cases 161→6, bailed 0→164. **Env knobs:** `REPAIR_MAX_ITERATIONS` (default 2), `REPAIR_NOPROGRESS_BAIL` (default on), `REPAIR_BAIL_ON_SIGNATURE` (default OFF/opt-in), `REPAIR_BAIL_MIN_ITERATION` (default 0). r20 (cap=2 + signature+code bail) and r22 (cap=3 + code-only bail) were tested and REJECTED — r20's signature clause cut off still-converging repairs (broke DB064/070/080); r22's 3rd repair over-repairs (McNemar p=0.049). cap=2 + code-only bail is the kept config.
2. **Stream HTTP response** (perceived-UX, zero accuracy risk) in `backend/src/api/classify.ts` + frontend.
3. **Cutover:** flip `USE_V2_CLASSIFIER` ON (staged); v2 becomes the live API.
4. **Frontend rebuild (`frontend/src/lib/hooks/use-wizard.ts`) — MUST be planned with user first:** read `alternatives`, handle `responseType:refused`, multi-turn `/answer` with `{questionId, answerId}` → publish + trade intelligence. Deferred quality lever: calibrated escalation to route low-confidence best-effort emits to ASK (would recover the confident-wrong delta).
5. **Brain ceiling-raisers (measurement-gated):** retrieval bucket (per-subheading leaf floor, RRF fusion, chapter-recall safety net); confidence calibration (ECE) → sharper ASK gate; deferred: fine-tuned domain reranker w/ sibling hard-negative mining, corpus re-embed with discriminating attributes, RAG over Indian ITC-HS advance rulings.
