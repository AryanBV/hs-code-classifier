...
| `contraband` | Goods that are ILLEGAL to TRADE AT ALL and have NO ITC-HS tariff line in normal commerce: narcotic drugs and psychotropic substances, and CITES Appendix-I raw wildlife products whose commercial trade is banned outright (raw ivory, tiger and rhino parts). Use this ONLY when the item itself cannot lawfully be a tariff good. | "heroin", "raw ivory tusks", "tiger bone powder" |
| `weapons_restricted_class` | Specific arms/munitions whose export is prohibited or governed by a separate licensing regime outside ITC-HS classifier scope | "anti-personnel landmines", "fissile uranium-235" — Ch.93 small arms ARE classifiable, do NOT refuse those |

**Critical carve-out — export-RESTRICTED or export-PROHIBITED but tariff-classifiable goods are NOT `contraband`. CLASSIFY them.** Many ordinary, lawful commodities carry a DGFT export restriction, prohibition, quota, or State-Trading channel. That is a POLICY status sitting on a real ITC-HS tariff line; the good is still perfectly classifiable and has its own code. These MUST route to CLASSIFY, never REFUSE — the downstream pipeline surfaces the export-policy status and the legal caveats on the result. Treat the legality of EXPORT as a policy attribute of a real good, NOT as a reason the good is non-classifiable. **Reserve `contraband` strictly for goods that are illegal to TRADE at all and have no tariff line (narcotics, CITES App-I raw wildlife).** Examples that MUST CLASSIFY (do NOT tag `contraband`): beef / meat of bovine animals, including where its export is prohibited (Ch.02, headings 0201/0202/0206/0210); sandalwood and red sanders (Ch.44); basmati and non-basmati rice, wheat, onions, sugar (subject to DGFT bans or quotas); ozone-controlled substances such as CFC/HCFC refrigerants (Ch.29, e.g. heading 2903 — they have real tariff lines and are merely export-controlled). The litmus test: if the item has a legitimate ITC-HS tariff line and the ONLY issue is whether it may be EXPORTED, CLASSIFY it.
