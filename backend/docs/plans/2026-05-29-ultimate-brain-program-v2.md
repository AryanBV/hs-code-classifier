...
**Status:** ACTIVE plan-of-record. Supersedes the priority order in `AUTONOMOUS-CONTINUATION-2026-05-29.md` where they disagree (this is more correct after audit).
**Branch:** `feat/phase-4-pipeline-build`. **Runtime:** Vertex only (Cohere OFF, do not reintroduce).
...
| Metric | Now (r6, routing-conditional) | Realistic ultimate | Notes |
|---|---|---|---|
| Chapter (frozen denom) | ~92 | 95–96% | textile + confusing-pairs + notes + GIR-ladder |
| Heading | ~88 | 90–92% | |
| **8-digit OUTRIGHT (answered)** | ~68 | **78–83%** | bounded by tariff ambiguity + query info |
| 8-digit EFFECTIVE (secondary) | — | 85–90% | UX ceiling, reported with [realistic, oracle] range |
| Routing | ~86.5 | 90–93% | |
| confident-wrong (count) | unmeasured | → 0 indefensible | manual-reviewed every instance |
| Calibration | unmeasured | directional + Brier ↓ | ECE w/ bootstrap CI; never claim "<5%" |
| p95 latency / $ per query | ~unmeasured | ≤8s / ≤$0.012 | 3-sided gate sidecar |
...
### P0 — Trust Spine (HARDENED). Gate for everything; complete BEFORE P1 fan-out.
...
### P1 — Corpus-wide diagnosis (read-only fan-out), re-scoped.
...
- **"Other"-residual-by-elimination** L4 reasoning mode (a distinct selection path).
- **QGS axis-selection quality** (the ~10 wrong-axis cases) + needless-ask reduction + ASK→single-code convergence.
